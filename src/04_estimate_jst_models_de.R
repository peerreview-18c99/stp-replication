...
input <- input %>% 
  tidylog::mutate(dfm_file = case_when(dictionary == "Rauh" ~ paste0("data/parlspeech dfms/dfm_parlspeech_", 
                                                                     sample,
                                                                     "_noUmlaute_",
                                                                     dfm_version,
                                                                     ".rds"),
                                       dictionary != "Rauh" ~ paste0("data/parlspeech dfms/dfm_parlspeech_", 
                                                                     sample,
                                                                     "_",
                                                                     dfm_version,
                                                                     ".rds")))
...
input <- input %>% 
  tidylog::mutate(dictionary_file = case_when(dictionary == "LIWC" ~ "data/dictionaries/dict_liwc_de.rds",
                                              dictionary == "LSD" ~ "data/dictionaries/dict_lsd_de.rds",
                                              dictionary == "Rauh" ~ "data/dictionaries/dict_rauh_de.rds"))
...
# Save results
write_rds(jst_results, file = "data/jst_results_de.rds", compress = "xz")
